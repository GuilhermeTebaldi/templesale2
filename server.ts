...
async function ensurePostgresEstablishmentSchema() {
  if (!pgPool) {
    return;
  }
  ...
  for (const statement of establishmentStatements) {
...
async function initializePostgresDatabase() {
  ...
  if (!RUN_DATABASE_MIGRATIONS) {
    await ensurePostgresEstablishmentSchema();
    console.log(
      "PostgreSQL migrations disabled by RUN_DATABASE_MIGRATIONS; ensured TempleSale establishment schema only.",
    );
    return;
  }
